...
**Purpose**: Simple greeting endpoint returning "hello world" as a JSON payload.
...
**Side effects**: None. Pure read-only response.

**Implementation**: `src/lib/greeting.ts` — `greetingHandler()` exported and registered in `src/worker.tsx`.
