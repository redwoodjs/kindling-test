...
**Purpose**: Simple greeting endpoint returning "hello world".
...
**Implementation**: `greetingHandler()` in `src/lib/greeting.ts`. Registered before `render(Document, [...])` to bypass the React SSR pipeline.
